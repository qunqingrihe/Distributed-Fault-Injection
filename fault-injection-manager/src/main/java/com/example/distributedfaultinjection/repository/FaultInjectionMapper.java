...
public interface FaultInjectionMapper {
    ...
    @Insert("INSERT INTO fault_injection(faultType, targetNode) VALUES(#{faultType}, #{targetNode})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insertFaultInjection(FaultInjectionModel faultInjection);

    @Update("UPDATE fault_injection SET faultType = #{faultType}, targetNode = #{targetNode} WHERE id = #{id}")
